...
\subsection{\texttt{oneSentencePerLine}}
	...
	\announce{NEW}{one sentence per line} your file so that it puts one sentence per line.
	%
...
latexindent.pl multiple-sentences3 -m -l=manipulate-sentences.yaml
	...
	For example, if we use the \texttt{.tex} file in \cref{lst:multiple-sentences4} and run the command \begin{commandshell}
...
\subsection{Poly-switches}
	Every other field in the \texttt{modifyLineBreaks} field uses poly-switches, and can take one of \emph{five} \announce*{2017-08-21}*{blank line poly-switch} integer values: \begin{itemize}[font=\bfseries]
		...
		\item[3] \emph{add then blank line mode} \announce*{2017-08-21}{blank line poly-switch}: a line break will be added before or after the \emph{<part of thing>} under consideration, assuming that there is not already a line break before or after the \emph{<part of thing>}, followed by a blank line.
		      %%
	\end{itemize}
...
\end{commandshell} the output is as in \cref{lst:env-mlb-mod1,lst:env-mlb-mod2} respectively.
	...
	Let's \announce*{2017-08-21}{demonstration of blank line poly-switch (3)}  now change each of the \texttt{1} values in \cref{lst:env-mlb1,lst:env-mlb2} so that they are $3$ and save them into \texttt{env-mlb5.yaml} and \texttt{env-mlb6.yaml} respectively (see \cref{lst:env-mlb5,lst:env-mlb6}).
	%
...
\end{commandshell} the output is as in \cref{lst:env-mlb-mod7,lst:env-mlb-mod8}.
	...
	Let's \announce*{2017-08-21}{demonstration of blank line poly-switch (3)} now change each of the \texttt{1} values in \cref{lst:env-mlb7,lst:env-mlb8} so that they are $3$ and save them into \texttt{env-mlb11.yaml} and \texttt{env-mlb12.yaml} respectively (see \cref{lst:env-mlb11,lst:env-mlb12}).
	%
