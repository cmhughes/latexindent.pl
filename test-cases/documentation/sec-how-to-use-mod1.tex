...
\section{How to use the script}
 ...
 \announce{new}{perl module installer helper script} in particular, note that a module installer helper script is shipped with \texttt{latexindent.pl}.
...
latexindent.pl myfile.tex -y='modifyLineBreaks:environments:one:EndStartsOnOwnLine:3' -m
    \end{commandshell} \label{page:yamlswitch}You \announce*{2017-08-21}{the -y switch} can specify YAML settings from the command line using the \texttt{-y} or \texttt{--yaml} switch; sample demonstrations are given above.
	%
...
latexindent.pl -screenlog myfile.tex
      \end{commandshell}
	Using this%
	\announce{NEW}{screenlog} option tells \texttt{latexindent.pl} to output the log file to the screen, as well
...
cat myfile.tex | latexindent.pl
	...
	\announce{NEW}{STDIN allowed} allow input from STDIN, which means that you can pipe output from
	...
	Similarly, if you%
	\announce{NEW}*{no options/filename updated} simply type \texttt{latexindent.pl} at the command line, then it will expect (STDIN) input from the command line.
	%
	\begin{commandshell}
latexindent.pl
      \end{commandshell}

	Once you have finished typing your input, you can press \texttt{CTRL+D} to signify that your input has finished.
