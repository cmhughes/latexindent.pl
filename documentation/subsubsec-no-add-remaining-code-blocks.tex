...
\subsubsection{The remaining code blocks}
 ...
 \paragraph{keyEqualsValuesBracesBrackets}
  ...
  \end{itemize}
  See the \texttt{keyEqualsValuesBracesBrackets: name} field of the fine tuning section in
 ...
 \paragraph{namedGroupingBracesBrackets} This type of code block is mostly motivated by
  ...
  In particular, \texttt{latexindent.pl} considers \texttt{child} and \texttt{parent} 
  to be \texttt{namedGroupingBracesBrackets}\footnote{ You may like to
  verify this by using the \texttt{-t} option and checking \texttt{indent.log}! }.
  Referencing \cref{lst:child1:default}, note that the maximum indentation is three tabs,
  and these come from:
  \begin{itemize}
...
\begin{example}
 Consider the file given in \cref{lst:named1}, together with its
...
\end{commandshell}
 ...
 \begin{example}
 Consider the file given in \cref{lst:finetuning2},
...
\end{commandshell}
 ...
 \index{bibliography files} \index{regular expressions!delimiterRegEx}
...
latexindent.pl bib2.bib -l bibsettings1.yaml,bibsettings2.yaml -o=+-mod2 
 ...
 \paragraph{UnNamedGroupingBracesBrackets} occur in a variety of situations;
  ...
  criteria:
  \begin{itemize}
      \item it isn't a \texttt{command}, \texttt{keyEqualsValuesBracesBrackets} or \texttt{namedGroupingBracesBrackets}
   \item and it has at least one set of curly braces or square brackets (comments and line
         breaks allowed throughout).
  \end{itemize}
  ...
  Referencing \cref{lst:psforeach:default}, there are \emph{two} sets of unnamed
  braces. Note also that the maximum value of indentation is two tabs, and these come
  from:
  \begin{itemize}
  ...
  \begin{example}
      Starting with the file in \cref{lst:unnamed1} we receive the default output in \cref{lst:unnamed1:mod0}.
