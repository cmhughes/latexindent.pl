...
It is very easy to have conflicting poly-switches. The most poly-switch corresponding to the 
most-recently-found code block will be respected.
...
and consider the YAML settings given in
...
\end{commandshell}
 ...
 To understand it, let's talk about how \texttt{latexindent.pl} 
 has processed \cref{lst:nested-env}; specifically \texttt{latexindent.pl}:
 \begin{enumerate}
     ...
     \item does the poly-switch work for \texttt{two} instructed by \cref{lst:nested-env-mlb1}: remove 
         the line break before the end statement, and adds a line break \emph{after} the 
         end statement.
         \end{enumerate}
     \item does the poly-switch work for \texttt{one}
         instructed by \cref{lst:nested-env-mlb1} which is: remove line breaks 
         before the end statement, and add a line break \emph{after} the end statement;
 \end{enumerate}
