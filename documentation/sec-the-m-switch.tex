...
\subsection{Poly-switches}
    ...
    one of \emph{five}\announce{NEW}* integer values:
	\begin{itemize}[font=\bfseries]
		...
		\item[2] \emph{comment then add mode}: a comment symbol will be added, followed by a line break before or after the \emph{<part of thing>} under consideration, assuming that
            ...
            \item[3] \emph{add then blank line mode}\announce{NEW}: a line break will be added before or after the \emph{<part of thing>} under consideration, assuming that
...
\end{commandshell}
    ...
    Let's\announce{NEW}  now change each of the \texttt{1} values in \cref{lst:env-mlb1,lst:env-mlb2} so that they are $3$ and
...
\end{commandshell}
    ...
    Let's\announce{NEW} now change each of the \texttt{1} values in \cref{lst:env-mlb7,lst:env-mlb8} so that they are $3$ and
    ...
    Note that line breaks have been added as in \cref{lst:env-mlb-mod7,lst:env-mlb-mod8}, but that a \emph{blank line}
