...
\end{commandshell}
    ...
    \announce*{new}*{huge:overflow is now default}; the default value is \texttt{overflow}, which means that 
    words will \emph{not} be broken by the text wrapping routine, implemeneted by the \texttt{Text::Wrap}. 
    Other options for the \texttt{huge} field are \texttt{wrap} and \texttt{die}.
