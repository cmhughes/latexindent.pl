...
\subsection{Poly-switches}
	...
	one of \emph{five}%
	\announce{NEW}*{blank line poly-switch} integer values:
	\begin{itemize}[font=\bfseries]
		...
		\item[3] \emph{add then blank line mode}%
		      \announce{NEW}{blank line poly-switch}: a line break will be added before or after the \emph{<part of thing>} under consideration, assuming that
...
\end{commandshell}
	...
	Let's%
	\announce{NEW}{demonstration of blank line poly-switch (3)}  now change each of the \texttt{1} values in \cref{lst:env-mlb1,lst:env-mlb2} so that they are $3$ and
...
\end{commandshell}
	...
	Let's%
	\announce{NEW}{demonstration of blank line poly-switch (3)} now change each of the \texttt{1} values in \cref{lst:env-mlb7,lst:env-mlb8} so that they are $3$ and
