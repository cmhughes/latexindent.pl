...
\NewDocumentCommand{\announce}{ s m o }{%
	\IfBooleanTF{#1}
	{% \announce*
		\tcbdocmarginnote[overlay={\node[cmhstar,scale=0.45] at ([yshift=0mm,xshift=1mm]frame.north east) {}; }]{\tcbdocnew{#2}}
      \addcontentsline{new}{subsection}{#3}%
	}
	{% \announce
...
\DeclareTCBInputListing[use counter=lstlisting]{\cmhlistingsfromfile}{s O{} s m O{} m m}{%
	...
	IfBooleanTF={#1}{new-to-this-version}{},
	#5,
}
...
% new features list
%\newcommand{\listOfNewFeatures}{\@starttoc{new}}
\newcommand{\listOfNewFeatures}{\@starttoc{new}}
\contentsuse{newfeatures}{new}
% toc settings
\titlecontents*{subsection}% <paragaph>
  [3cm]% <left>
