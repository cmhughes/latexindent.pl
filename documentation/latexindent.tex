...
\usepackage{examplep}
% tcolorbox libraries
\tcbuselibrary{breakable,skins,listings,minted,}
...
listing engine=listings,
left=0cm,
every listing line={\textcolor{red}{\small\ttfamily\bfseries cmh:$\sim$\$ }}}
...
latexindent.pl myfile.tex --overwrite 
      \end{commandshell}
	      ...
	      more on this in \cref{sec:defuseloc}; see \texttt{backupExtension} and \texttt{onlyOneBackUp}.
	...
	\item[\verbitem{-o}] \texttt{=output.tex,--outputfile=output.tex}
      \begin{commandshell} 
...
latexindent.pl -d myfile.tex
	...
	\item[\verbitem{-c, --cruft=<directory>}]
      \begin{commandshell}
...
latexindent.pl myfile.tex -g other.log 
	...
	\item[\verbitem{-m, --modifylinebreaks}] 
      \begin{commandshell}
...
\end{document}
\begin{itemize}
	\item[\verbitem{defaultIndent}] \texttt{"\t"}

	      This is the default indentation (\texttt{\t} means a tab) used in the absence of other details
	...
	\item[\verbitem{backupExtension}] \texttt{.bak}
	      ...
	      any indentation: \texttt{myfile.bak0}

	      By default, every time you call \texttt{latexindent.pl} after this with
	      the \texttt{-w} switch it will create \texttt{myfile.bak1}, \texttt{myfile.bak2},
	      etc.
	\item[\verbitem{onlyOneBackUp}] \texttt{0}
	      ...
	      then change \texttt{onlyOneBackUp} to \texttt{1}.

	\item[\verbitem{maxNumberOfBackUps}]\texttt{0}
	...
	\item[\verbitem{cycleThroughBackUps}]\texttt{0}
	      ...
	      set to \texttt{1}  then the \texttt{copy} procedure given in \cref{lst:cycleThroughBackUps}
	      would be obeyed.

	      \begin{cmhlistings}[language=Perl]{\texttt{cycleThroughBackUps}}{lst:cycleThroughBackUps}
...
copy myfile.bak4 to myfile.bak3
	...
	\item[\verbitem{indentPreamble}] \texttt{0}

	      The preamble of a document can sometimes contain some trickier code
	      for \texttt{latexindent.pl} to work with. By default, \texttt{latexindent.pl}
	      won't try to operate on the preamble, but if you'd like it to try then
	      change \texttt{indentPreamble} to \texttt{1}.
	\item[\verbitem{alwaysLookforSplitBraces}] \texttt{1}

	      This switch tells \texttt{latexindent.pl} to look for commands that
	      can split \emph{braces} across lines, such as \texttt{parbox}, \texttt{tikzset}, etc. In older
	      versions of \texttt{latexindent.pl} you had to specify each one in \texttt{checkunmatched}--this
	      clearly became tedious, hence the introduction of \texttt{alwaysLookforSplitBraces}.
	      ...
	      \emph{As long as you leave this switch on (set to 1) you don't need to specify which
	      	...
	      	fields \texttt{checkunmatched} and \texttt{checkunmatchedELSE} described
	      	later on \cpageref{lst:checkunmatched}}.
	\item[\verbitem{alwaysLookforSplitBrackets}] \texttt{1}

	      This switch tells \texttt{latexindent.pl} to look for commands that
	      can split \emph{brackets} across lines, such as \texttt{psSolid}, \texttt{pgfplotstabletypeset},
	      etc. In older versions of \texttt{latexindent.pl} you had to specify each one in \texttt{checkunmatchedbracket}--
	      this clearly became tedious, hence the introduction of \texttt{alwaysLookforSplitBraces}.

	      \emph{As long as you leave this switch on (set to 1) you don't need to specify which
	      	commands can split brackets across lines--you can ignore \texttt{checkunmatchedbracket} described later on
	      	\cpageref{lst:checkunmatched}}.

	\item[\verbitem{removeTrailingWhitespace}] \texttt{0}

	      By default \texttt{latexindent.pl} indents every line (including empty lines)
	      which creates `trailing white space' feared by most version control systems. If
	      this option is set to \texttt{1}, trailing white space is removed from all
	...
	\item[\verbitem{lookForAlignDelims}] This is the first example of a field
	      in \texttt{defaultSettings.yaml} that has more than one line; \cref{lst:aligndelims:basic}
	      ...
	      \begin{cmhlistings}[style=yaml]{\texttt{lookForAlignDelims} (basic)}{lst:aligndelims:basic}
lookForAlignDelims:
   tabular: 1
   tabularx: 1
   longtable: 1
   ...
   listabla: 1
	...
	\begin{minipage}{.5\textwidth}
		\begin{cmhlistings}[style=demo,columns=fixed]{\texttt{tabular} before}{lst:tabularbefore:basic}
\begin{tabular}{cccc}
1&	2 &3       &4\\
5& &6       &\\
\end{tabular}
	...
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\begin{cmhlistings}[style=demo,columns=fixed]{\texttt{tabular} after (basic)}{lst:tabularafter:basic}
\begin{tabular}{cccc}
 1 & 2 & 3 & 4 \\
 5 &   & 6 &   \\
\end{tabular}
	...
	environments then you can either remove them from \texttt{lookForAlignDelims} altogether, or set the relevant key to \texttt{0}, for example \texttt{tabular: 0}, or if you just want to ignore \emph{specific}
	instances of the environment, you could wrap them in something from \texttt{noIndentBlock} (see \cref{lst:noIndentBlock}).

    If you wish to remove the alignment of the \texttt{\\} within a delimiter-aligned block, then the
    advanced form of \texttt{lookForAlignDelims} shown in \cref{lst:aligndelims:advanced} is for you. 
	      \begin{cmhlistings}[style=yaml]{\texttt{lookForAlignDelims} (advanced)}{lst:aligndelims:advanced}
lookForAlignDelims:
   tabular: 
      delims: 1
   ...
   longtable: 1
    ...
    \begin{itemize}
      ...
      \item \texttt{alignDoubleBackSlash}: switch to determine if \texttt{\\} should be aligned (default: 1);
      \item \texttt{spacesBeforeDoubleBackSlash}: optionally, specifies the number of spaces to be inserted
        before (non-aligned) \texttt{\\}. In order to use this field, \texttt{alignDoubleBackSlash} needs
    ...
    the \texttt{\\} have not (compare the alignment of \texttt{\\} in \cref{lst:tabularafter:basic,lst:tabularafter:advanced}).

	\begin{minipage}{.5\textwidth}
		\begin{cmhlistings}[style=demo,columns=fixed]{\texttt{tabular} before }{lst:tabularbefore:advanced}
\begin{tabular}{cccc}
1&	2 &3       &4\\
5& &6       &\\
\end{tabular}
	...
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\begin{cmhlistings}[style=demo,columns=fixed]{\texttt{tabular} after (advanced)}{lst:tabularafter:advanced}
\begin{tabular}{cccc}
 1 & 2 & 3 & 4\\
 5 &   & 6 &\\
\end{tabular}
    ...
    note the spacing before the \texttt{\\} in \cref{lst:tabularafter:spacing}.

	\begin{minipage}{.5\textwidth}
		\begin{cmhlistings}[style=demo,columns=fixed]{\texttt{tabular} before}{lst:tabularbefore:spacing}
\begin{tabular}{cccc}
1&	2 &3       &4\\
5& &6       &\\
\end{tabular}
	...
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\begin{cmhlistings}[style=demo,columns=fixed]{\texttt{tabular} after (spacing)}{lst:tabularafter:spacing}
\begin{tabular}{cccc}
 1 & 2 & 3 & 4   \\
 5 &   & 6 &   \\
\end{tabular}
	...
	If you have blocks of code that you wish to align at the \&  character that
	      are \emph{not} wrapped in, for example, \texttt{\begin{tabular}...\end{tabular}}, then you use the mark up
...
%* \end{tabular}
	...
	\item[\verbitem{verbatimEnvironments}] A field that contains a list of environments
	      ...
	      \begin{cmhlistings}[style=yaml]{\texttt{verbatimEnvironments}}{lst:verbatimEnvironments}
verbatimEnvironments:
	...
	\item[\verbitem{noIndentBlock}] If you have a block of code that you don't
	      want \texttt{latexindent.pl} to touch (even if it is \emph{not} a verbatim-like
	      ...
	      \begin{cmhlistings}[style=yaml]{\texttt{noIndentBlock}}{lst:noIndentBlock}
noIndentBlock:
    noindent: 1
    cmhtest: 1
	\end{cmhlistings}
	...
	in your code, so you use them with a comment symbol, \texttt{%}, followed
...
\end{enumerate}
	...
	\item[\verbitem{indentAfterHeadings}] This field enables the user to specify
	      indentation rules that take effect after heading commands such as \texttt{\part}, \texttt{\chapter},
	      \texttt{\section}, \texttt{\subsection*} etc. This field is slightly different from most
	      ...
	      \begin{cmhlistings}[style=yaml]{\texttt{indentAfterHeadings}}{lst:indentAfterHeadings}
indentAfterHeadings:
    part:
       indent: 0
       level: 1
	...
	can easily switch them on by changing \texttt{indent: 0} to \texttt{indent: 1}.
	...
	as appropriate.  You can also specify your own indentation in \texttt{indentRules}--
	you will find the default \texttt{indentRules} contains \texttt{chapter: " "} which
	...
	\item[\verbitem{indentAfterItems}] The environments specified in \texttt{indentAfterItems}  tell
	      \texttt{latexindent.pl} to look for \texttt{\item} commands; if these switches are set to \texttt{1}
	      then indentation will be performed so as indent the code after each \texttt{item}.
	      \begin{cmhlistings}{\texttt{indentAfterItems}}{lst:indentafteritems}
indentAfterItems:
    itemize: 1
    enumerate: 1
	\end{cmhlistings}
	...
	\begin{minipage}{.5\textwidth}
		\begin{cmhlistings}[style=demo,xleftmargin=-3mm,columns=fixed]{\texttt{items} before}{lst:itemsbefore}
\begin{itemize}
\item some text here
some more text here
some more text here
...
\end{itemize}
	...
	\end{minipage}%
	\begin{minipage}{.5\textwidth}
		\begin{cmhlistings}[style=demo,xleftmargin=-3mm,columns=fixed]{\texttt{items} after}{lst:itemsafter}
\begin{itemize}
	\item some text here
		   some more text here
		   some more text here
...
\end{itemize}
	...
	\item[\verbitem{itemNames}] If you have your own \texttt{item} commands (perhaps you
...
\fi
	...
	\item[\verbitem{fileExtensionPreference}] \texttt{latexindent.pl} can be called to
	      act on a file without
	      specifying the file extension.  For example we can call \texttt{latexindent.pl myfile}
	      ...
	      \begin{cmhlistings}[style=yaml]{\texttt{fileExtensionPreference}}{lst:fileExtensionPreference}
fileExtensionPreference:
    ...
    .bib: 4
	\end{cmhlistings}
	Calling \texttt{latexindent.pl myfile} with the details specified in \cref{lst:fileExtensionPreference}
...
\item[\verbitem{fileContentsEnvironments}] 
  \texttt{latexindent.pl} determines when the main document begins by looking for \texttt{\begin{document}}; 
  it will not do so when inside any of the environments specified in \texttt{fileContentsEnvironments}, see
  \cref{lst:fileContentsEnvironments}.
\begin{cmhlistings}[style=yaml]{\texttt{fileContentsEnvironments}}{lst:fileContentsEnvironments}
fileContentsEnvironments:
  ...
  \end{cmhlistings}
	...
	\item[\color{red}\verbitem{checkunmatched}] Assuming you keep \texttt{alwaysLookforSplitBraces} set to \texttt{1} (which
	      is the default) then you don't need to worry about \texttt{checkunmatched}.

	      Should you wish to deactivate \texttt{alwaysLookforSplitBraces} by setting it to \texttt{0}, then
	      you can populate \texttt{checkunmatched} with commands that can split braces across
	      lines--see \cref{lst:checkunmatched}.

	      \begin{cmhlistings}[style=yaml]{\texttt{checkunmatched}}{lst:checkunmatched}
checkunmatched:
    parbox: 1
    vbox: 1
	\end{cmhlistings}
	\item[\color{red}\verbitem{checkunmatchedELSE}] Similarly, assuming you keep \texttt{alwaysLookforSplitBraces} set to \texttt{1} (which
	      is the default) then you don't need to worry about \texttt{checkunmatchedELSE}.

	      As in \texttt{checkunmatched}, should you wish to deactivate \texttt{alwaysLookforSplitBraces} by setting it to \texttt{0}, then
	      you can populate \texttt{checkunmatchedELSE} with commands that can split braces across
	      lines \emph{and} have an `else' statement--see \cref{lst:checkunmatchedELSE}.

	      \begin{cmhlistings}[style=yaml]{\texttt{checkunmatchedELSE}}{lst:checkunmatchedELSE}
checkunmatchedELSE:
    pgfkeysifdefined: 1
    DTLforeach: 1
    ifthenelse: 1
	\end{cmhlistings}
	\item[\color{red}\verbitem{checkunmatchedbracket}] Assuming you keep \texttt{alwaysLookforSplitBrackets}
	      set to \texttt{1} (which is the default) then you don't need to worry about \texttt{checkunmatchedbracket}.

	      Should you wish to deactivate \texttt{alwaysLookforSplitBrackets} by setting it
	      to \texttt{0}, then you can populate \texttt{checkunmatchedbracket} with commands that can
	      split \emph{brackets} across lines--see \cref{lst:checkunmatchedbracket}.

	      \begin{cmhlistings}[style=yaml]{\texttt{checkunmatchedbracket}}{lst:checkunmatchedbracket}
...
\begin{cmhlistings}[style=demo,escapeinside={(*@}{@*)}]{When to set \texttt{alwaysLookforSplitBrackets=0}}{lst:casestudy}
...
\end{shortsolution}
\end{cmhlistings}
I had allowed \texttt{alwaysLookforSplitBrackets=1}, which is the default setting.
Unfortunately, this caused undesired results, as \texttt{latexindent.pl} thought that the opening
\texttt{[}  in the interval notation (\cref{line:interval1,line:interval2})
was an opening brace that needed to be closed (with a corresponding \texttt{]}). Clearly
this was inappropriate, but also expected since \texttt{latexindent.pl} was simply
following its matching rules.

In this particular instance, I set up \texttt{localSettings.yaml}
to contain \texttt{alwaysLookforSplitBrackets: 0} and then specified the commands
that could split brackets across lines (such as \texttt{begin{axis}}) individually
in \texttt{checkunmatchedbracket}.  Another option would have been to wrap the
the line in an environment from \texttt{noIndentBlock} which treats its contents
