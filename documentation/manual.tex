	...
	overlay={
		...
		decorate[decoration={stars,segment length=2cm}] {
			decorate[decoration={zigzag,segment length=2cm,amplitude=0.3cm}] {
				([xshift=-.5cm,yshift=0.1cm]frame.south west) --  ([xshift=-.5cm,yshift=0.6cm]frame.north west)
		}};
		\draw[inner sep=0,minimum size=rnd*15pt+2pt]
		decorate[decoration={stars,segment length=2cm}] {
			decorate[decoration={zigzag,segment length=2cm,amplitude=0.3cm}] {
...
% \begin{noindent}
	\title{\lstinline[basicstyle=\huge\ttfamily]!latexindent.pl!\\[1cm]
		Version 2.1R}
% \end{noindent}
\author{Chris Hughes \footnote{and contributors! (See \vref{sec:knownlimitations}.)}}
\maketitle
\begin{abstract}
...
\begin{enumerate}
	...
	\item \lstinline!localSettings.yaml! (if found in same directory as \lstinline!myfile.tex! and called
	      with \lstinline!-l! switch; can be renamed, provided that the call to \lstinline!latexindent.pl! is adjusted 
          accordingly)
\end{enumerate}
...
\printbibliography[env=specialbib,heading=bibintoc,title={Contributors},keyword=contributor]
