...
\flagbox{-s, --silent} \index{switches!-s, --silent
...
\subsection{switches via yaml}

 \announce*{2026-03-15}{switchesViaYAML details}For some of the switches, you can specify
 them by the YAML interface; the defaults are shown in \cref{lst:switchesViaYaml}.

 Note: switches can only be turned \emph{on} using this field, they will not be turned
 \emph{off}.

 \cmhlistingsfromfile*[style=switchesViaYaml]{../defaultSettings.yaml}[width=.95\linewidth,before=\centering,yaml-TCB]{\texttt{switchesViaYaml}}{lst:switchesViaYaml}
