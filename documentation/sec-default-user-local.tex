...
\yamltitle{logFilePreferences}*{fields}
	...
	When%
	\announce{NEW}{showDecorationStartCodeBlockTrace} either of the \texttt{trace} modes (see \cpageref{page:traceswitch})
	...
	\announce{NEW}{log file pattern layout} uses the \texttt{log4perl} module \cite{log4perl}
   ...
   ...
	...
	\begin{itemize}
      \item \texttt{delims}: binary switch (0 or 1) equivalent to simply specifying, for example, \texttt{tabular: 1} in
            ...
            \item \texttt{spacesBeforeDoubleBackSlash}: optionally,\announce{NEW}*{update to spacesBeforeDoubleBackSlash} specifies the number (integer $\geq$ 0) of spaces to be inserted
        ...
        \item \announce{2017-06-19}{multiColumnGrouping} \texttt{multiColumnGrouping}: binary switch (0 or 1) that details if \texttt{latexindent.pl} should group columns
            ...
            \item \announce{NEW}{spacesBeforeAmpersand}\texttt{spacesBeforeAmpersand}: optionally specifies the number (integer $\geq$ 0) of 
              spaces to be placed \emph{before} ampersands (default: 1);
            \item \announce{NEW}{spacesAfterAmpersand}\texttt{spacesAfterAmpersand}: optionally specifies the number (integer $\geq$ 0) of 
    ...
    and the YAML files in \crefrange{lst:tabular2YAML}{lst:tabular7YAML}.
...
latexindent.pl tabular2.tex -l tabular2.yaml,tabular7.yaml
        \end{commandshell}
    we obtain the respective outputs given in \crefrange{lst:tabular2-default}{lst:tabular2-mod5}.
