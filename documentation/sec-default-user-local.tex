   ...
   ...
	...
	\begin{itemize}
		...
		\item \announce{NEW}{dontMeasure}{don't measure feature} optionally specifies if
		      ...
		      measured (default: 0);
		\item \announce{NEW}{delimiterRegEx}{delimiter RegEx feature} optionally specifies the pattern
...
\subsection{lookForAlignDelims: the dontMeasure feature}\label{sec:dontMeasure}
	The%
	\announce{NEW}{don't measure feature} \texttt{lookForAlignDelims} field can, optionally, receive the
	\texttt{dontMeasure} option which can be specified in a few different ways. We will
	explore this feature in relation to the code given in \cref{lst:tabular-DM}; the
...
\subsection{lookForAlignDelims: the delimiterRegEx feature}\label{sec:delimiter-reg-ex}
	The delimiter alignment%
	\announce{new}{delimiterRegEx feature} will, by default, align code blocks at
	the ampersand character. The behaviour is controlled by the \texttt{delimiterRegEx} field
	within \texttt{lookForAlignDelims}; the default value is \lstinline3'(?<!\\)(&)'3, which can
	be read as: \emph{an ampersand, as long as it is not immediately preceeded by a backslash}.
...
\end{commandshell}
	...
	\end{itemize}
	We can explore this a little further using the settings in \cref{lst:delimiterRegEx2} and
	run the command
	\begin{commandshell}
