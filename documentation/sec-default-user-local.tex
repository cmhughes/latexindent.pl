...
\yamltitle{logFilePreferences}*{fields}
    ...
    Some of the options \announce{new}*{logFilePreferences updated to include Dumper options}for Perl's \texttt{Dumper} module can be specified in \cref{lst:logFilePreferences};
    see \cite{dumper} and \cite{dumperdemo} for more information; these options will mostly 
    be helpful for those calling \texttt{latexindent.pl} with the \texttt{-tt} option described 
...
\yamltitle{noIndentBlock}*{fields}
	...
	\cmhlistingsfromfile{demonstrations/noindentblock.tex}{\texttt{noIndentBlock.tex}}{lst:noIndentBlock}

	Important note: it is assumed that the \texttt{noindent} block statements appear on
	their own line.

	The%
	\announce{new}{noIndentBlock specified as regex} \texttt{noIndentBlock}
...
\subsubsection{lookForAlignDelims: spacesBeforeAmpersand}
	...
	\announce{new}*{spacesBeforeAmpersand leading blank column upgrade} can be specified in a few different
...
\end{commandshell}
	...
	optional;%
	\announce{new}*{lookForThis optional for specialBeginEnd} by default, it is
