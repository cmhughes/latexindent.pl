...
\section{Config}\label{sec:defuseloc}
 \texttt{latexindent.pl} loads its default settings from \texttt{defaultSettings.yaml}. The idea
 ...
 \LaTeX. \emph{All of these settings can be changed by you, the user; demonstrations
 are given throughout this documentation using the -l switch}.

 If you look in \texttt{defaultSettings.yaml} you'll find the settings that govern the
...
\yamltitle{lookForAlignDelims}*{fields}
 ...
 \cmhlistingsfromfile*[style=lookForAlignDelims]{../defaultSettings.yaml}[before=\centering,yaml-TCB]{\texttt{lookForAlignDelims} (basic)}{lst:aligndelims:basic}
...
latexindent.pl tabular1.tex   
 ...
 \cmhlistingsfromfile*{demonstrations/tabular-adv.yaml}[width=.9\linewidth,before=\centering,yaml-TCB]{\texttt{lookForAlignDelims} (advanced)}{lst:aligndelims:advanced}

 Note that you can use a mixture of the basic and advanced form: in
 \cref{lst:aligndelims:basic} the entries are basic; in \cref{lst:aligndelims:advanced}
 the entry for \texttt{tabular} is advanced. \announce*{2026-03-15}{fineTuning controls
 defaults for advanced form of lookForAlignDelims} The default values for the advanced
 form are controlled in \texttt{fineTuning}, see
 \vref{subsec:finetuning:lookForAlignDelims}. When using the advanced form, each field
 should receive at least 1 sub-field, and \emph{can} (but does not have to) receive any
 ...
 files in \crefrange{lst:tabular2YAML}{lst:tabular8YAML}; the \texttt{dontMeasure}
...
\subsubsection{lookForAlignDelims: other}
...
\yamltitle{indentAfterItems}*{fields}
 ...
 for example) then you can put adjust \texttt{itemRegEx} in the \texttt{fineTuning}
 field, discussed in
 \vref{subsec:finetuning:itemsRegex}.\announce*{2026-03-15}{itemsRegex controls item}
...
\yamltitle{specialBeginEnd}*{fields}\label{yaml:specialBeginEnd}
 ...
 \announce*{2026-03-15}{specialBeginEnd specified as list}%
...
latexindent.pl special1.tex -o=+-default
 ...
 Now consider the YAML file shown in \cref{lst:specialsLeftRight-yaml}
...
\end{commandshell}
 ...
 \end{widepage}
 we receive the output in \cref{lst:specialLR-comm-first-tex}.
...
\end{commandshell}
 ...
 Using the settings in \cref{lst:special-nested1} and running the command
...
\end{commandshell}
 ...
 \begin{cmhtcbraster}[ raster left skip=-3.5cm,]
  \cmhlistingsfromfile{demonstrations/special-body-mod1.tex}{\texttt{special-body.tex} using \cref{lst:special-nested1}}{lst:special-body-mod1}
  \cmhlistingsfromfile*[style=yaml-LST]{demonstrations/special-nested1.yaml}[yaml-TCB]{\texttt{special-nested1.yaml}}{lst:special-nested1}
 \end{cmhtcbraster}
 We note that the output in \cref{lst:special-body-mod1} is as we would expect, as the
 \texttt{nested} field has been specified in \cref{lst:special-nested1}.
 \announce*{2026-03-15}{specialBeginEnd nested demonstration}
 \end{example}
...
\yamltitle{indentAfterHeadings}*{fields}
 ...
 \lstinline!\subsection*!, or indeed any user-specified command written in this field.
 ...
 switch them on by changing \texttt{lookForThis} from 0 to 1. The \texttt{level} field
 tells \texttt{latexindent.pl} the hierarchy of the heading structure in your document.
 You might, for example, like to have both \texttt{section} and \texttt{subsection} set
 with \texttt{level: 3} because you do not want the indentation to go too deep.
...
\subsection{Arguments and the strings allowed between them}
 ...
 are controlled by \texttt{fineTuning}; specific demonstrations are given in \vref{subsec:finetuning:arguments}.

\subsection{The code blocks known to latexindent.pl}\label{subsubsec:code-blocks}
...
\fi
  \end{lstlisting}                                                                                                                                                                                                                                                                      \\\cmidrule{2-3}
    items                         & User specified, see \vref{lst:indentafteritems}                                             &
    \begin{lstlisting}[,nolol=true,]
\begin{enumerate}
  \item ...
\end{enumerate}
