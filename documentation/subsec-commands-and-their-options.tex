...
\end{commandshell}
	...
	\announce{new}*{amalgamate feature in commandCodeBlocks}, we see that the first field in
...
\end{commandshell}
	...
	\begin{minipage}{.45\textwidth}
		\cmhlistingsfromfile*{demonstrations/for-each-mod1.tex}{\texttt{for-each.tex} using \cref{lst:no-strings}}{lst:for-each-mod1}
	\end{minipage}
	\hfill
	\begin{minipage}{.49\textwidth}
    ...
    You might like to compare the output given in \cref{lst:for-each-default} and \cref{lst:for-each-mod1}; in particular, in 
    \cref{lst:for-each-mod1} the \texttt{foreach} command has been allowed to have \lstinline!\x/\y! and \texttt{in} between 
