...
\section{\texttt{indentconfig.yaml} and \texttt{.indentconfig.yaml} (for user settings)}\label{sec:indentconfig}
...
\end{yaml}
	...
	and adds the changes we described on \cpageref{page:examsettings}; you might also like to
...
\subsection{Settings load order}\label{sec:loadorder}
	...
	\begin{enumerate}
		...
		\item \texttt{localSettings.yaml} but only if found in the same directory as \texttt{myfile.tex} and called
		      ...
		      accordingly (see \cref{sec:localsettings}). You may specify relative \fixthis{and absolute??!} paths to other
