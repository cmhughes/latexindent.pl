...
\subsection{localSettings.yaml and friends}\label{sec:localsettings}
	...
	\texttt{localSettings.yaml} and friends in the \emph{same directory} as \texttt{myfile.tex}.  For
	\announce{NEW}*{-l switch: localSettings and friends}
	example, if you use the following command
	\index{switches!-l demonstration}
	\begin{commandshell}
latexindent.pl -l myfile.tex
\end{commandshell}
	then \texttt{latexindent.pl} will search for and then, assuming they exist, load each of the 
...
\item .latexindent.yaml
\end{enumerate}
	These files will be assumed to be in the same directory as \texttt{myfile.tex}, or otherwise in 
    the current working directory. In what follows, whenever we refer to \texttt{localSettings.yaml}
