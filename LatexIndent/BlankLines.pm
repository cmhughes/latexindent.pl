...
sub condense_blank_lines{
    ...
    if(${$masterSettings{modifyLineBreaks}}{preserveBlankLines}==0){
        ...
        $self->logger("Updating body to inclued blank line token, this requires preserveBlankLines = 1",'ttrace') if($self->is_tt_switch_active);
        $self->logger("(any blanklines that could have been removed, would have done so by this point)",'ttrace') if($self->is_tt_switch_active);

        # make the call
        $self->protect_blank_lines ;
        $self->logger("body now looks like:\n${$self}{body}",'ttrace') if($self->is_tt_switch_active);
     }
    ...
    $self->logger("blank line replacement token: $replacementToken",'ttrace') if($self->is_tt_switch_active);
